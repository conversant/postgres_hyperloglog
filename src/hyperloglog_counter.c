...
hyperloglog_merge_simple(PG_FUNCTION_ARGS)
{
    ...
    HyperLogLogCounter counter1 = NULL;
    HyperLogLogCounter counter2 = NULL;
    ...
    if (PG_ARGISNULL(0) && PG_ARGISNULL(1)) {
        ...
        counter1 = hyperloglog_merge(counter1, counter2, false);
    }
...
hyperloglog_get_estimate(PG_FUNCTION_ARGS)
{
    ...
    double estimate;
    HyperLogLogCounter hyperloglog = PG_GETARG_HLL_P_COPY(0);
...
{
	hyperloglog_reset_internal((PG_GETARG_HLL_P(0)));
	PG_RETURN_VOID();
}
...
hyperloglog_send(PG_FUNCTION_ARGS)
{
    ...
    StringInfoData buf;
    pq_begintypsend(&buf);
    pq_sendbytes(&buf,(char *) bp, VARSIZE(bp) );
